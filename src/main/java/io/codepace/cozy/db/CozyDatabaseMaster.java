...
public class CozyDatabaseMaster {
        ...
        try {
            ...
            while (scan.hasNextLine()) {
                ...
                if (toAdd.validateBlock(this.blockchain)) {
                    ...
                    System.out.println("[CRITICAL ERROR] BLOCK " + toAdd.blockNum + " NOT VALID BUT IN BLOCKCHAIN DB FILE!");
                }
            }
            scan.close();
        } catch (Exception e) {
    ...
    /**
     * Returns the balance of an address on the Cozycoin network. Does not include unconfirmed balances.
     *
    ...
    /**
     * Returns the signature index of an address on the Cozycoin network. Does not account for previous signatures; only blockchain info.
     *
